...
**PrivateLink endpoints without endpoint policies.** An Interface Endpoint with no endpoint policy is wide open — any principal who can reach the endpoint can call any action on the service. For the card vault use case, the endpoint policy should explicitly allow only `kms:Decrypt` and `kms:GenerateDataKey` to the payment processor's execution role. Without this, a compromised Lambda in the same VPC could call `kms:ScheduleKeyDeletion`. Endpoint policies are a second access control layer on top of IAM.
